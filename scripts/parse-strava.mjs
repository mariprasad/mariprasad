...
// Manual tracks for activities whose Strava archive shipped no GPS file. The
// [lat, lon] waypoints trace the real corridor; densify() fills points between
// them so the route draws and maps like any other.
const MANUAL_TRACKS = {
  // Ride to Gudibanda (137 km) — Judicial Layout → Gudibande up the NH44 corridor
  // (Devanahalli → Chikkaballapur → Gudibande). No GPS in the export; this is an
  // approximation of the highway, not a recorded trace.
  "9453574948": [
    [13.0840, 77.5760], // Judicial Layout, Bengaluru
    [13.2490, 77.7120], // Devanahalli
    [13.4355, 77.7315], // Chikkaballapur
    [13.6680, 77.6990], // Gudibande
  ],
};

// Interpolate [lat,lon] waypoints into ~target evenly-spaced points.
function densify(waypoints, target) {
  if (!waypoints || waypoints.length < 2) return null;
  const segs = [];
  let total = 0;
  for (let i = 1; i < waypoints.length; i++) {
    const a = { lat: waypoints[i - 1][0], lon: waypoints[i - 1][1] };
    const b = { lat: waypoints[i][0], lon: waypoints[i][1] };
    const d = hav(a, b);
    segs.push({ a: waypoints[i - 1], b: waypoints[i], d });
    total += d;
  }
  const pts = [waypoints[0]];
  for (const s of segs) {
    const n = Math.max(1, Math.round((target * s.d) / total));
    for (let k = 1; k <= n; k++) {
      const t = k / n;
...
for (const r of data) {
  ...
  const id = r[ci("Activity ID")];
  const fileRel = r[ci("Filename")] || "";
  let shaped = null;
  if (/\.gpx$/i.test(fileRel) && fs.existsSync(`${SRC}/${fileRel}`)) {
    shaped = shapeRoute(parseGpx(`${SRC}/${fileRel}`)); // real recorded track
  } else if (MANUAL_TRACKS[id]) {
    shaped = densify(MANUAL_TRACKS[id], MAX_PTS); // synthesized from waypoints
  }
  if (!shaped) continue;
  ...
  routes.push({
    id,
    name: (r[ci("Activity Name")] || type).trim(),
    type,
...
// ---- personal bests: best of each type across ALL activities (incl. the
// route-less ones, e.g. the 137 km Gudibanda ride that ships without a track). ----
// Pre-format the date in the ride's own (machine-local) time so the displayed
// day is correct no matter the build server's timezone.
const prettyLocal = (s) => {
  const d = new Date(s);
...
const acts = data.map((r) => ({
  id: r[ci("Activity ID")],
  type: r[ci("Activity Type")] || "Other",
  name: (r[ci("Activity Name")] || r[ci("Activity Type")] || "Activity").trim(),
  date: prettyLocal(r[ci("Activity Date")]),
  distanceKm: Math.round(num(r[ci("Distance")]) * 10) / 10,
  elevationM: Math.round(num(r[ci("Elevation Gain")])),
}));
const bestBy = (pool, sel) => pool.reduce((b, a) => (!b || sel(a) > sel(b) ? a : b), null);
const records = [];
const distRecord = (label, type) => {
  const a = bestBy(acts.filter((x) => x.type === type), (x) => x.distanceKm);
  if (a && a.distanceKm > 0) records.push({ id: a.id, label, type, value: a.distanceKm, unit: "km", name: a.name, date: a.date });
};
distRecord("Longest ride", "Ride");
// Longest run is already surfaced in "Movement in numbers", so skip it here.
distRecord("Longest walk", "Walk");
...
const body =
  ...
  `export type StravaRecord = { id: string; label: string; type: string; value: number; unit: string; name: string; date: string };\n` +
  `export const STRAVA_RECORDS: StravaRecord[] = ${JSON.stringify(records)};\n\n` +
